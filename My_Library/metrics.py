...
def total_pages_covered(cluster_name,ground_truth):
        ...
        for elem in ground_truth:
            for e in cluster_name:
                if (elem==e[1]):
                    if(elem in total_pg_covered.keys()):
                        total_pg_covered[elem]=total_pg_covered[elem]+e[2]
                    else:
                        total_pg_covered[elem]=0
                        total_pg_covered[elem]=total_pg_covered[elem]+e[2]
        return total_pg_covered
...
def accuracy(path):
        ...
        pages_c=pages_covered(clusters, ground_truth)
        total_pg_covered=total_pages_covered(pages_c,ground_truth)
        accuracy={}
        for key in file_counter.keys():
            for elem in total_pg_covered.keys():
                if(key==elem):
                    accuracy[key]=total_pg_covered[elem]/file_counter[key]
